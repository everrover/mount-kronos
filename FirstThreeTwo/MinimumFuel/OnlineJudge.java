package FirstThreeTwo.MinimumFuel;
...
//    Map<Integer, List<RoadTo>> roadToMap;
    private int dijkstrasAlgorithmWithStateMap(int citiesCount, int fuelTankCapacity, Map<Integer, List<RoadTo>> roadToMap, int [][]costsToReachFromTo, int [][]pathsToReachFromTo, Map<Integer, Integer> fuelCosts, int qStart, int qEnd){
        Queue<State> pq = new PriorityQueue<>();
        pq.add(new State(qStart, 0, 0));
        while(!pq.isEmpty()){
            State state = pq.poll();
        ...
        for(int dist: costsToReachFromTo[qEnd]){ res = Math.min(res, dist); }
        return res;
    }

    private int getMinimumFuel(
            int citiesCount, int qStart, int qEnd,
            int fuelTankCapacity,
...
//        Arrays.fill(costsToReachFromTo[qStart], 0); costsToReachFromTo[qStart][0] = Integer.MAX_VALUE;
    ...
    public static void main(String[] args) {
        ...
        while(q-->0){
            ...
            int c = new OnlineJudge().getMinimumFuel(n, scanner.nextInt(), scanner.nextInt(), capacity, travelFuelRequirements, fuelCosts);
            System.out.println(c==-1?"Impossible": c);
        }
    }
}
